...
class Game:
    ...
    def __init__(self):
        ...
        self.player1_ai: int = -1  # -1: Human, 0: Random, 4,5,6: Minimax depths, -2: RL
        self.player2_ai: int = -1
        self.win_player: int = 0
        self.selected_cell = -1
        self.ai_is_playing: bool = False
        self.rl_agent = RLAgent(self)  # Initialize RL agent
        self.cells = [Cell(self, 0, (95, 320), (70, 70), self.dan_count[0], self.quan_count[0])]
        self.cells += [Cell(self, i + 1, (170 + 75 * i, 357.5), (70, 70), self.dan_count[i + 1]) for i in range(5)]
        self.cells += [Cell(self, 6, (545, 320), (70, 70), self.dan_count[6], self.quan_count[1])]
        self.cells += [Cell(self, i + 7, (170 + 75 * (4 - i), 282.5), (70, 70), self.dan_count[i + 7]) for i in range(5)]
        self.point_cells = [
            Point_Cell(self, 1, (120, 520), (70, 70), 0, 0),
        ...
        self.reset_button = pygame_gui.elements.UIButton(
            relative_rect=reset_btn_rect, text="Reset", manager=self.manager, object_id="#reset_btn"
        )
        p1_options_surf = pygame.Surface((150, 50))
        p1_options_rect = p1_options_surf.get_rect(center=(320, 520))
        self.p1_options = pygame_gui.elements.UIDropDownMenu(
            options_list=['Human', 'AI-0', 'AI-4', 'AI-5', 'AI-6', 'AI-RL'],
        ...
        self.p2_options = pygame_gui.elements.UIDropDownMenu(
            options_list=['Human', 'AI-0', 'AI-4', 'AI-5', 'AI-6', 'AI-RL'],
            starting_option='Human',
    ...
    def move(self, dan_count, index, is_clockwise):
        ...
        while dan_count[current_index] == 0 and dan_count[clamp(current_index + step)] != 0:
            if (clamp(current_index + step) in [0, 6]
                    and (self.quan_count[clamp(current_index + step) % 5] != 0
                         and dan_count[clamp(current_index + step)] < Game.DAN_LIMIT)):
                pass
            else:
    ...
    async def ai_play(self, is_right):
        # Set sleep time to 0 to train faster and set to 1 for normal game
    ...
    async def handle_AI(self):
        ...
        if self.player1_turn:
            ...
            if self.player1_ai == 0:
            ...
            elif self.player1_ai == -2:  # RL agent
                state = self.rl_agent.get_state()
                position, direction = self.rl_agent.play(state, self.player1_turn)
                self.selected_cell = position
                self.cells[self.selected_cell].select_cell()
                if direction == 1:
                    self.cells[self.selected_cell].arrow_cells[1].select_cell()
                ...
                new_board, new_quan_count, new_point, position, direction = Ai.player(
                    board=self.dan_count,
                    quan_count=self.quan_count,
                    point_count=[
                        self.point_count[0][1] * Game.QUAN_COEFFICIENT + self.point_count[0][0],
            ...
            if self.player2_ai == 0:
                ...
                if direction == 1:
                    ...
                    self.cells[self.selected_cell].arrow_cells[0].select_cell()
                asyncio.create_task(self.ai_play(direction > 0))
            elif self.player2_ai == -2:  # RL agent
                state = self.rl_agent.get_state()
                position, direction = self.rl_agent.play(state, self.player1_turn)
                self.selected_cell = position
                self.cells[self.selected_cell].select_cell()
                ...
                new_board, new_quan_count, new_point, position, direction = Ai.opponent(
                    board=self.dan_count,
                    quan_count=self.quan_count,
                    point_count=[
                        self.point_count[0][1] * Game.QUAN_COEFFICIENT + self.point_count[0][0],
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame_gui.UI_DROP_DOWN_MENU_CHANGED:
                if event.ui_element == self.p1_options:
                    if event.text == 'Human':
                        self.player1_ai = -1
                    elif event.text == 'AI-RL':
                        self.player1_ai = -2
                        self.rl_agent.load_q_table()
                    else:
                        self.player1_ai = int(event.text[-1]) if event.text != 'AI-0' else 0
                    ...
                    if event.text == 'Human':
                        ...
                        self.player2_ai = int(event.text[-1]) if event.text != 'AI-0' else 0
